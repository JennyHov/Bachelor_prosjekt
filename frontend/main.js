      ...
      data.items.forEach((event, index) => {
        ...
        eventArticle.innerHTML = `
          ...
          <div class="p-4 md:p-6 lg:p-8 grid gap-4 md:gap-3">
            <p class="text-slate-400 text-sm">${eventTime}</p>
            <h2 class="font-bold text-2xl">${event.summary}</h2>
            <button class="toggle-details text-left text-slate-400 hover:text-blue-800 focus:outline-none" aria-expanded="false" data-details-id="details-${index}">
              See details
            </button>
